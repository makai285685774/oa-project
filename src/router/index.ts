...
router.beforeEach(async (to, from, next) => {
	...
	if (to.path === '/login' && !token) {
		...
		if (!token) {
			...
			if (JSON.parse(sessionStorage.getItem('userMessage') || '').number == '2') {
				next('/home');
				// router.addRoute({
				// 	path: '/home',
				// 	name: 'home',
				...
				//   });
			} else if (JSON.parse(sessionStorage.getItem('userMessage') || '').number == '1') {
				next('/home1');
			}else if (JSON.parse(sessionStorage.getItem('userMessage') || '').number == '3') {
				next('/home2');
			} else {
				next('/home'); 
			}
